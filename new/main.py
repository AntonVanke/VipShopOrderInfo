...
# 判断是否有 log 文件
FIRST_START = not os.path.exists("LICENSE")
...
def update_users():
    ...
    info_text_label.config(text=_info)
    return True
...
if __name__ == '__main__':
    ...
    # 第一次启动弹出协议
    if FIRST_START:
        if messagebox.askokcancel("使用协议", "1.开源程序\n请点击确认同意协议"):
            logger.info("*****您已同意协议*****")
            with open("LICENSE", "w", encoding="utf-8") as _license:
                pass
        else:
            root.destroy()
    ...
    export_data_button = tk.Button(tools_labelframe, text="更新所有数据并导出订单", command=update_orders_excel,
                                   fg="#ffaf0f")
    update_all_button = tk.Button(tools_labelframe, text="更新所有数据", command=update_orders, fg="#2a42b4")
    export_all_data_button = tk.Button(tools_labelframe, text="仅导出数据库订单",
